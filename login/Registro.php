<?php
    ...
    if(!empty($_POST['nombre']) && !empty($_POST['password']) && !empty($_POST['email'])){
        $records = $conn->prepare('SELECT * FROM usuarios1 WHERE email = :email');
        $records->bindParam(':email', $_POST['email']);
        $records->execute();
        $results = $records->fetch(PDO::FETCH_ASSOC);

        if(strcmp($_POST['email'], $results['email']) == 0){
            $message = 'Usuario ya registrado';
            $code = 1;
        }

        if($code != 1){
            $sql = "INSERT INTO usuarios1 (nombre, contrasenia, email, fk_rol) VALUES(:nombre, :contrasenia, :email, 1)";
        ...
        <article class="registro-contenido">
            <div class="registro-contenido-izq">
                ...
                <?php if(!empty($message)): ?>
                    <p> <?= $message ?></p>
                <?php endif; ?>                
                <form id="form-registro" action="Registro.php" class="registro-izq-form" method="POST">
                    <div>
